...
int Spawn(const char *program, const char *command, struct Kernel_Thread **pThread)
{
	...
	if ((result = Read_Fully(program, (void **) &exeFileData, &exeFileLength))
        != 0) {
		Print("Failed to read file %s!\n", program);
		goto fail;
	}

	if ((result = Parse_ELF_Executable(exeFileData, exeFileLength, &exeFormat))
        != 0) {
		Print("Failed to parse elf file!\n");
		goto fail;
	}

	if ((result = Load_User_Program(exeFileData, exeFileLength, &exeFormat,
